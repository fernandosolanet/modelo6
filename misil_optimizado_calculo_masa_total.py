...
for gasto in [14]:
    ...
    archivo.write('Altura lanzamiento \tAngulo lanz'
    ...
    for i in range(130, 131, 1):
        ...
        v_orb = 7800
        v_loss = 867.6923503
        # Velocidad con la que empieza a iterar
        v_loss_empirica = 0
        error = 800
        contador = 1
        #
        ...
        while error > 10 and contador <= 50:
            ...
            # 'inputs_iniciales'
            vl = v0 + v_rotacional
            Ml = (vl - v_rotacional) / ((GAMMA * R_AIR * temperature(zl))**.5)
            vxl = vl * sin(fil)
...
#        archivo.write('%.8f\t' % error)
#        archivo.write('%.8f\n' % i)

#        if zl > 500000:
#            break
        elapsed_time1 = time() - start_time0
        print("Tiempo de ejecución: %0.3f segundos." % elapsed_time1)
    elapsed_time = time() - start_time
    print("Tiempo de ejecución: %0.3f segundos." % elapsed_time)
    archivo.close()
