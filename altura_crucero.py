...
def crucero(PESO_SI, mach):

    C_TRAMOS1 = []
    C_TRAMOS2 = []
    C_TRAMOS3 = []

    PESO = PESO_SI/0.453592
    P1 = 42777.0
    P2 = 43035.0
    P3 = 45472.0
    P4 = 46279.0
    ...
    if mach_enve[0] <= mach <= dom_1:
        altura = (C_1[0] + C_1[1]*mach + C_1[2]*mach**2 +
                  C_1[3]*mach**3 + C_1[4]*mach**4 + C_1[5]*mach**5)*1000*0.3048
        return altura

    elif dom_1 < mach <= dom_2:
        altura = (C_2[0] + C_2[1]*mach + C_2[2]*mach**2 +
                  C_2[3]*mach**3 + C_2[4]*mach**4)*1000*0.3048
        return altura

    elif dom_2 < mach <= mach_enve[-1]:
        altura = (C_3[0] + C_3[1]*mach + C_3[2]*mach**2 +
                  C_3[3]*mach**3 + C_3[4]*mach**4 + C_3[5]*mach**5 +
                  C_3[6]*mach**6)*1000*0.3048
        return altura

    elif mach_enve[-1] < mach:
        return print('El mach introducido no pertenece a la envolvente.')

    elif mach < mach_enve[0]:
        return print('El mach introducido no pertenece a la envolvente.')
